...
function showSearchResults(searchResults){
    ...
    for(var j=0;j<searchResults.goods.length;j++){
        res+='<div class="search-goods search-result"><a href="'+searchResults.goods[i].link+'">'+searchResults.goods[i].name+'<a></div>'
    }
...
function search(searchValue, db) {
    ...
    for (var i = 0; i < db.goods.length; i++) {
        if((db.goods[i].name).toUpperCase().indexOf(s)>=0){
            searchResults.goods.push({"name":db.goods[i].name, "link":db.goods[i].url})
        }
    }
